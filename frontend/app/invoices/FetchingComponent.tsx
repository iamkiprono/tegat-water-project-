...
const FetchingCOmponent = async ({
  ...
  const response = async (): Promise<(typeof farmerType)[]> => {
    const url = process.env.NEXT_PUBLIC_URL;

    // setLoading(true);
    // const res = await fetch("https://milimani-api.onrender.com/farmers/bills");
    const res = await fetch(
      `${url}/farmers/bills?page=${1}&month=${month}&year=${year}&type=INVOICE`
    );
